...
os.makedirs("aa")
print( pytesseract.image_to_string(Image.open('aa.png'),lang='ara'))
...
img1 =img
cv2.imshow('i',img)
img = cv2.erode(img, kernel1, iterations=2)
print(kernel1)
...
for (j, w) in enumerate(res):
    ...
    crop_img = img1[y-1:y + h+1, x-1:x + w+1]
    Matrix[i][0] = y
    Matrix[i][1] =y+h
    Matrix[i][2] =x
    Matrix[i][3] = x+w
    Matrix[i][4] = 'aa/pic000%d.png' % (i,)
    #img.save('temp2.png')
    #cv2.imshow('pic{:>05}.jpg'.format(i), crop_img)
    cv2.imwrite('aa/pic{:>05}.png'.format(i), crop_img)
    i += 1
    print(x)

images = glob.glob("aa/*.jpg")


# j=0
# for image in images:
#     arabic01 = cv2.imread(image, 1)
#     d=arabic_processing(arabic01, False)
#     cv2.imwrite('dd/pic{:>05}.jpg'.format(j), d)
#     j +=1
k=0
arr=[]
y="يشير"
yarab = glob.glob("aa/*.png")
for image in yarab  :
